...
for s_path in glob.glob('output0*'):
    ...
    mcdsts = pcdl.TimeSeries(s_path, settingxml=f'PhysiCell_settings_episode{str(i_episode).zfill(3)}.xml')
    ...
    mcdsts.plot_timeseries(None, 'substrate', ax=axs[1])
    mcdsts.plot_timeseries('cell_type', 'substrate', ax=axs[2])
